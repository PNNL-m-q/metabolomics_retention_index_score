...
NormScore <- function(`Retention Index`, `Retention Index Ref`, Mean, SD, `Compound Name`, `Retention Index Score`) {
...
Normal <- MQ_Holdout %>%
  merge(Parameters[,c("Compound Name", "Normal Mean", "Normal SD")], by = "Compound Name", all.x = T) %>%
  mutate(
    `New RI Score` = pmap(list(`Retention Index`, `Retention Index Ref`, `Normal Mean`, `Normal SD`, 
                               `Compound Name`, `Retention Index Score`), NormScore) %>% unlist()
  )

# Define a function to calculate new ranks for a metabolite 
holdout_test_normal <- function(metabolite) {
  
  # Get the frequency 
  Freq <- Normal[Normal$`Compound Name` == metabolite, "N"][1] %>% unlist()
  ...
  Dist <- Normal %>%
    ...
    unlist()
  LocalNormParams <- tryCatch({fitdistr(Dist, "normal")}, error = function(e) {return(NULL)})
  
  if (is.null(LocalNormParams)) {
    
    FinalNorm <- Normal
    FinalNorm[,c("Normal Mean", "Normal SD", "New RI Score", "New RI Rank")] <- NA
    
  } else {
    
    # Pull parameters
    newNorm <- LocalNormParams$estimate[1]
    newSD <- LocalNormParams$estimate[2]
    
    FinalNorm <- Normal %>%
      mutate(`New RI Score` = pmap(
        list(`Retention Index`, `Retention Index Ref`, newNorm, newSD, `Compound Name`, `Retention Index Score`),
        function(`Retention Index`, `Retention Index Ref`, newNorm, newSD, `Compound Name`, `Retention Index Score`) {
          ifelse(`Compound Name` == metabolite, 
                 NormScore(`Retention Index`, `Retention Index Ref`, newNorm, newSD, `Compound Name`, `Retention Index Score`), 
                 `New RI Score`)
        }) %>% unlist()
      ) %>%
      group_by(`Sample Name`, `Peak Index`) %>%
...
for (i in 1:length(Compounds)) {
  ...
  results <- foreach (j=1:50, .combine = rbind) %dopar% {
    holdout_test_normal(Compounds[i])
  }
  results$Iteration <- rep(1:50, each = nrow(results)/50)
  fwrite(results, file.path("~/Git_Repos/metabolomics_scoring_metrics/Retention_Index/RI_Specific_Data/Normal_Prob_Holdout/", paste0(Compounds[i], ".txt")),
...
data.frame(
  ...
  trelliscope(path = "~/Git_Repos/metabolomics_scoring_metrics/Retention_Index/Markdowns/4-Retention-Index-Distribution-Adjustment-Rank/Normal_Probability_Holdout/", 
...
GammaScore <- function(`Compound Name`, `Retention Index`,
                       GammaShape, GammaRate, `Retention Index Score`) {
...
Gamma <- MQ_Holdout %>%
  ...
  mutate(
    `New RI Score` = pmap(list(`Compound Name`, `Retention Index`,
                               `Gamma Shape`, `Gamma Rate`, `Retention Index Score`), GammaScore) %>% unlist()
  )
...
holdout_test_gamma <- function(metabolite) {
  ...
  if (is.null(LocalGammaParams)) {
    ...
    FinalGamma <- Gamma %>%
      mutate(`New RI Score` = pmap(
        list(`Compound Name`, `Retention Index`, newShape, newRate, `Retention Index Score`, `New RI Score`),
        function(`Compound Name`, `Retention Index`, newShape, newRate, `Retention Index Score`, `New RI Score`) {
          ifelse(`Compound Name` == metabolite, 
                 GammaScore(`Compound Name`, `Retention Index`, newShape, newRate, `Retention Index Score`), 
                 `New RI Score`)
        }) %>% unlist()
      ) %>%
      group_by(`Sample Name`, `Peak Index`) %>%
...
data.frame(
  ...
  trelliscope(path = "~/Git_Repos/metabolomics_scoring_metrics/Retention_Index/Markdowns/4-Retention-Index-Distribution-Adjustment-Rank/Gamma_Holdout", 
              name = "True Positive", nrow = 1, ncol = 1, thumb = T)
data.frame(
  "Compound" = Compounds
) %>%
...
LogisticScore <- function(`Compound Name`, `Retention Index`, `Logistic Location`, `Logistic Scale`, `Retention Index Score`) {
  
  if (`Compound Name` %in% Compounds) {
    
    # Then determine which side of the probability distribution the value falls on,
    # and calculate the score.
    ScoreVal <- plogis(`Retention Index`, `Logistic Location`, `Logistic Scale`)
    
    if (is.na(ScoreVal)) {return(`Retention Index Score`)}
...
Logistic <- MQ_Holdout %>%
  ...
  mutate(
    `New RI Score` = pmap(list(`Compound Name`, `Retention Index`, `Logistic Location`, `Logistic Scale`, `Retention Index Score`), LogisticScore) %>% unlist()
  )
...
holdout_test_logistic <- function(metabolite) {
  ...
  if (is.null(LocalLogisParams)) {
    ...
    FinalLogis <- Logistic %>%
      mutate(`New RI Score` = pmap(
        list(`Compound Name`, `Retention Index`, newLocation, newScale, `Retention Index Score`, `New RI Score`),
        function(`Compound Name`, `Retention Index`, newLocation, newScale, `Retention Index Score`, `New RI Score`) {
          ifelse(`Compound Name` == metabolite, 
                 LogisticScore(`Compound Name`, `Retention Index`, newLocation, newScale, `Retention Index Score`), 
                 `New RI Score`)
        }) %>% unlist()
      ) %>%
      group_by(`Sample Name`, `Peak Index`) %>%
...
LNormScore <- function(`Compound Name`, `Retention Index`, `Log Mean`, `Log SD`, `Retention Index Score`) {
  
  if (`Compound Name` %in% Compounds) {
    
    # Then determine which side of the probability distribution the value falls on,
    # and calculate the score.
    ScoreVal <- plnorm(`Retention Index`, `Log Mean`, `Log SD`)
    if (ScoreVal > 0.5) {ScoreVal <- 1 - ScoreVal}
    return(round(ScoreVal * 2, 8))
...
LogNormal <- MQ_Holdout %>%
  ...
  mutate(
    `New RI Score` = pmap(list(`Compound Name`, `Retention Index`, `Log Normal Mean`, `Log Normal SD`, `Retention Index Score`), LNormScore) %>% unlist()
  )
...
holdout_test_lnorm <- function(metabolite) {
  ...
  if (is.null(LocalLognormParams)) {
    ...
    FinalLNorm <- LogNormal %>%
      mutate(`New RI Score` = pmap(
        list(`Compound Name`, `Retention Index`, newLNormMean, newLNormSD, `Retention Index Score`, `New RI Score`),
        function(`Compound Name`, `Retention Index`, newLNormMean, newLNormSD, `Retention Index Score`, `New RI Score`) {
          ifelse(`Compound Name` == metabolite, 
                 LNormScore(`Compound Name`, `Retention Index`, newLNormMean, newLNormSD, `Retention Index Score`), 
                 `New RI Score`)
        }) %>% unlist()
      ) %>%
      group_by(`Sample Name`, `Peak Index`) %>%
