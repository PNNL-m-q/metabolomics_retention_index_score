...
library(forcats); library(parallel); library(doParallel)
```
...
param_fit <- foreach(cmpd = Compounds) %dopar% {
  ...
  suppressWarnings({list(
    ...
    "Logistic" = tryCatch({fitdistr(distr, "logistic")}, error = function(e) {return(NA)}),
...
```{r}
AIC_Format %>%
  filter(`Scaled AIC Bin` == "[0,2]") %>%
  ungroup() %>%
  dplyr::select(Distribution) %>%
  unlist() %>%
  table(dnn = "Distribution")
```
...
```{r}
TP_Rank1 <- Ranks %>% 
  filter(Truth == "True Positive" & Rank == 1) %>% 
  dplyr::select(`Compound Name`, Proportion, Score) %>% 
  mutate(Score = ifelse(Score != "Original", "Other", "Original")) %>% 
  group_by(`Compound Name`) %>% 
  filter(Proportion == max(Proportion)) %>% 
...
TN_Rank1 <- Ranks %>% 
  filter(Truth == "True Negative" & Rank == 1) %>% 
  dplyr::select(`Compound Name`, Proportion, Score) %>% 
  mutate(Score = ifelse(Score != "Original", "Other", "Original")) %>% 
  group_by(`Compound Name`) %>% 
  filter(Proportion == min(Proportion)) %>% 
...
# Make table
data.table(
  Truth = c(rep("True Positive", 2), rep("True Negative", 2)),
  Rank = c(1,5,1,5),
  Original = c(sum(TP_Rank1$Score == "Original"), sum(TP_Rank5$Score == "Original"),
               sum(TN_Rank1$Score == "Original"), sum(TN_Rank5$Score == "Original")),
  Other = c(sum(TP_Rank1$Score == "Other"), sum(TP_Rank5$Score == "Other"),
            sum(TN_Rank1$Score == "Other"), sum(TN_Rank5$Score == "Other")),
  `All 0` = c(8, 1, 62, 41)
) %>%
  mutate(Ties = 87 - Original - Other - `All 0`)
```
